//! Plugins are an internal experimental interface for extending the
//! [Workspace].
...
/// A workspace plugin which comes from a corresponding
/// [WorkspacePluginConfig::setup]. In that setup call, the plugin will have
/// initial access to the whole [Workspace], and will be able to add listeners
/// to changes to blocks in the [Workspace].
pub(crate) trait PluginImpl: 'static {
    /// IDEA 1/10:
    /// This update is called sometime between when we know changes have been
    /// made to the workspace and the time when we will get the plugin to
    /// query its data (e.g. search())
...
pub(crate) struct PluginMap {
    /// We store plugins into the TypeMap, so that their ownership is tied to
    /// [Workspace]. This enables us to properly manage lifetimes of
    /// observers which will subscribe into events that the [Workspace]
    /// experiences, like block updates.
