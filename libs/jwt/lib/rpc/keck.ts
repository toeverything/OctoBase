...
const setupWS = (provider: KeckProvider) => {
	if (provider.shouldConnect && provider.ws === null) {
		...
		websocket.onclose = (event: any) => {
			...
			setTimeout(setupWS, math.min(math.pow(2, provider.wsUnsuccessfulReconnects) * 100, provider.maxBackOffTime) + provider.extraToleranceTime, provider)
		}
		websocket.onopen = () => {
			provider.wsLastMessageReceived = Date.now()
			provider.wsconnecting = false
...
export class KeckProvider extends Observable<string> {
	...
	constructor(
		...
		this._checkInterval = /** @type {any} */ setInterval(() => {
			if (this.wsconnected && messageReconnectTimeout < Date.now() - this.wsLastMessageReceived) {
