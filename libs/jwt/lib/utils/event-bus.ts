...
class BlockEventBus {
	...
	constructor(workspace: string, eventBus: EventTarget, cache?: EventBusCache) {
	...
	public type<T = unknown>(
		...
		if (!this._cache.scope.has(topic)) {
			const bus = new TopicEventBus<T>(this._workspace, topic, this._eventBus, this._cache)
...
class TopicEventBus<T = unknown> extends BlockEventBus {
	...
	constructor(workspace: string, topic: string, eventBus: EventTarget, cache: EventBusCache) {
		super(workspace, eventBus, cache)
		this._topic = topic
	}

	on(name: string, listener: ((e: T) => Promise<void>) | ((e: T) => void), options?: ListenerOptions) {
		if (options?.debounce) {
