...
import { RemoteBinaries } from './binary.js'
import { YBlock } from './block.js'
import { GateKeeper } from './gatekeeper.js'
import { HistoryManager } from './history.js'
import type { YProviderFactory } from './provider.js'
import type { ChangedStateKeys, Connectivity } from './types.js'
...
function initYProvider(
	...
	if (provider && (provider.userId === options.userId || options.userId === 'default')) {
		return provider
	}
	...
	const gatekeeper = new GateKeeper(
		userId,
		gateKeeperData.get('creators') || gateKeeperData.set('creators', new YMap()),
	...
	if (options.providers) {
		...
		synced = Promise.all(
			Object.entries(options.providers).flatMap(([name, provider]) => [
				provider({ awareness, doc, token, workspace, emitState }).catch((e) => {
					console.error(`Failed to connect to ${workspace} with ${name} provider: ${e}`)
				}),
				// p({
				//     awareness,
				//     doc: binaries,
...
export type { YBlock } from './block.js'
export { HistoryManager } from './history.js'
export { getYProviders } from './provider.js'
export type { YProviderOptions, YProviderType } from './provider.js'
...
export class YBlockManager {
	...
	constructor(workspace: string, options: YInitOptions) {
		const { eventBus, userId = 'default', token, providers: provider } = options
		...
		this._reload = () => {
			...
			// @ts-ignore
			this._history = new HistoryManager(this._blocks, this._eventBus.type('history'))
			...
			this._blocks.observeDeep((events) => {
				...
				const keys = events.flatMap((e) => {
					...
					if ((e.path?.length | 0) > 0) {
						return [[e.path[0], 'update'] as [string, ChangedStateKeys]]
					}
					return Array.from(e.changes.keys.entries()).map(([k, { action }]) => [k, action] as [string, ChangedStateKeys])
				})
		...
		const debouncedEditingNotifier = debounce(
			() => {
				...
				for (const { userId, editing, updated } of mapping.values()) {
					...
					if (userId && editing && updated && typeof updated === 'number' && updated + 1000 * 10 > Date.now()) {
				...
				this._eventBus
					.type('system')
					.topic('editing')
					.emit(new Map(Object.entries(editingMapping).map(([k, v]) => [k, new Set(v)])))
			},
			200,
			{ maxWait: 1000 }
		)
	...
	inspector() {
		const resolveBlock = (blocks: Record<string, any>, id: string) => {
			...
			if (block) {
				return {
					...block,
					children: block.children.map((id: string) => resolveBlock(blocks, id)),
				}
			}
		}
	...
	public checkBlocks(keys: string[]): boolean {
		return keys.filter((key) => !!this._blocks.get(key)).length === keys.length
	}
