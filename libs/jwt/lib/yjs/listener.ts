...
import type { ChangedStateKeys } from './types.js'

export function ChildrenListenerHandler(eventBus: TopicEventBus, event: YEvent<any>, topic?: string) {
	const keys = Array.from(event.keys.entries()).map(([key, { action }]) => [key, action] as [string, ChangedStateKeys])
...
export function ContentListenerHandler(eventBus: TopicEventBus, events: YEvent<any>[], topic?: string) {
	const keys = events
		.flatMap((e) => {
			...
			return Array.from(e.changes.keys.entries()).map(([k, { action }]) => [k, action] as [string, typeof action])
		})
