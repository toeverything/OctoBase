...
import { IndexedDBProvider, KeckProvider } from '../rpc/index.js'
import type { BucketBackend } from '../types/index.js'
...
export const getYProviders = (options: YProviderOptions): Record<string, YProviderFactory> => {
	// eslint-disable-next-line no-console
	console.log('getYProviders', options)
	return {
		indexeddb: async (context: YContext) => {
			if (options.enabled.includes('idb')) {
				await new IndexedDBProvider(context.workspace, context.doc).whenSynced
			}
		},
		keck: async (context: YContext) => {
			if (options.enabled.includes('keck')) {
				const ws = new KeckProvider(options.backend, context.workspace, context.doc, {
				...
				return new Promise((resolve, reject) => {
					...
					ws.on('lost-connection', () => context.emitState('retry'))
					ws.on('connection-error', () => context.emitState('retry'))
				})
			}
		},
	}
