...
export type HandleErrorFunc = (errorCode: string, errorMsg: string, cancelApply: AnyFunc, stopPropagation: () => void) => void

export type HandleError = (errorCode: string, errorMsg: string) => boolean

export abstract class AbstractCommand {
	public abstract validate(store: JwtStore, handleError: HandleError, cancelApply: () => void): boolean

	public abstract apply(store: JwtStore, dispatchOperation: (op: Operation) => unknown, handleError: HandleError, cancelApply: () => void): boolean
}

// eslint-disable-next-line @typescript-eslint/consistent-type-definitions
export interface IStoreClient {
	next(command: AbstractCommand | AbstractCommand[] | (() => AbstractCommand[])): IStoreClient
