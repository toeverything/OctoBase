...
import { getLogger, nanoid } from '../utils/index.js'
import type { HistoryManager, YBlock } from '../yjs/index.js'
import type { BlockListener, BlockPosition, ChangedStateKeys, ChangedStates } from '../yjs/types.js'
import type { BlockCapability } from './capability.js'
...
type GetExporter<R> = (block: YBlock) => Readonly<[string, ReadableContentExporter<R>]>[]
...
export class AbstractBlock<T extends object = object> {
	...
	private readonly _contentExportersGetter: () => Map<string, ReadableContentExporter<string>>

	private readonly _metadataExportersGetter: () => Map<string, ReadableContentExporter<Array<[string, number | string | string[]]>>>

	private readonly _tagExportersGetter: () => Map<string, ReadableContentExporter<string[]>>
	...
	constructor(block: YBlock, bus: TopicEventBus, root?: AbstractBlock, parent?: AbstractBlock, exporters?: Exporters) {
		this._id = block.id
		this._block = block
		...
		this._metadataExportersGetter = () => new Map(exporters?.metadata(block))
	...
	public on(event: 'content' | 'children' | 'parent', name: string, callback: BlockListener) {
		if (event === 'parent') {
			this.eventBus.topic<CallbackData>('parentChange').on(name, callback)
		} else {
	...
	private _emitParent(parentId: string, type: 'update' | 'delete' = 'update') {
		const states: Map<string, 'update' | 'delete'> = new Map([[parentId, type]])
	 ...
	 * @returns
	 */
	public async insertChildren(block: AbstractBlock, position?: BlockPosition) {
	...
	private _insertBlocks(parentNode: AbstractBlock | undefined, blocks: AbstractBlock[], placement: 'before' | 'after', referenceNode?: AbstractBlock) {
		if (!blocks || blocks.length === 0 || !parentNode) {
			return
		}
		// TODO: array equal
		if (!referenceNode && parentNode.children.join('') === blocks.map((node) => node.id).join('')) {
			return
		}
		blocks.forEach((block) => {
			...
			const placementInfo = {
				[placement]:
					referenceNode?.id || (parentNode.hasChildNodes() && parentNode.children[placement === 'before' ? 0 : parentNode.children.length - 1]),
			}

			parentNode.insertChildren(block, placementInfo[placement] ? placementInfo : undefined)
		})
	}
