...
use jwst::{Block as JwstBlock, Workspace, WorkspaceTransaction};
...
impl Block {
    ...
    pub fn children(&self) -> Vec<String> {
        self.workspace.with_trx(|trx| {
            self.block.children(&trx.trx)
        })
    }

    pub fn push_children(&self, block: &Block) {
        self.workspace.with_trx(|mut trx| self.block.push_children(&mut trx.trx, &block.block));
    }

    pub fn insert_children_at(&self, block: &Block, pos: u32) {
        self.workspace.with_trx(|mut trx| self.block.insert_children_at(&mut trx.trx, &block.block, pos));
    }
    ...
    pub fn insert_children_before(
        ...
        self.workspace
            .with_trx(|mut trx| self.block.insert_children_before(&mut trx.trx, &block.block, reference));
    }

    pub fn insert_children_after(&self, block: &Block, reference: &str) {
        self.workspace.with_trx(|mut trx| self.block.insert_children_after(&mut trx.trx, &block.block, reference));
    }

    pub fn remove_children(&self, block: &Block) {
        self.workspace.with_trx(|mut trx| self.block.remove_children(&mut trx.trx, &block.block));
    }

    pub fn exists_children(&self, block_id: &str) -> i32 {
        self.workspace.with_trx(|trx| self.block.exists_children(&trx.trx, block_id))
            .map(|i| i as i32)
            .unwrap_or(-1)
    }
    ...
    pub fn parent(&self) -> String {
        self.workspace.with_trx(|trx| {
            self.block.parent(&trx.trx).unwrap()
        })
    }

    pub fn updated(&self) -> u64 {
        self.workspace.with_trx(|trx| {
            self.block.updated(&trx.trx)
        })
    }
    ...
    pub fn created(&self) -> u64 {
        self.workspace.with_trx(|trx| {
            self.block.created(&trx.trx)
        })
    }

    pub fn set_bool(&self, key: String, value: bool) {
        self.workspace.with_trx(|mut trx| self.block.set(&mut trx.trx, &key, value));
    }
    ...
    pub fn set_null(&self, key: String) {
        self.workspace.with_trx(|mut trx| self.block.set(&mut trx.trx, &key, Any::Null));
    }

    pub fn is_bool(&self, key: String) -> bool {
        self.workspace.with_trx(|trx| {
            self.block.get(&trx.trx, &key).map(|a| matches!(a, Any::Bool(_))).unwrap_or(false)
        })
    }

    pub fn is_string(&self, key: String) -> bool {
        self.workspace.with_trx(|trx| {
            self.block.get(&trx.trx, &key).map(|a| matches!(a, Any::String(_))).unwrap_or(false)
        })
    }

    pub fn is_float(&self, key: String) -> bool {
        self.workspace.with_trx(|trx| {
            self.block.get(&trx.trx, &key).map(|a| matches!(a, Any::Number(_))).unwrap_or(false)
        })
    }

    pub fn is_integer(&self, key: String) -> bool {
        self.workspace.with_trx(|trx| {
            self.block.get(&trx.trx, &key).map(|a| matches!(a, Any::BigInt(_))).unwrap_or(false)
        })
    }
